...
# ### Manasi Gokhale <br>
# ### Christmas-themed computer programming challenges of Advent of Code

# In[54]:


#TASK 1/ PART 1 - Find the Elf carrying the most Calories. How many total Calories is that Elf carrying?
...
#process the input and convert it into list
individual_elf = input_data.strip().split('\n\n')

list_of_calories_per_elf = [list(map(int, ind_elf.split('\n'))) for ind_elf in individual_elf]

total_cal = [sum(calories) for calories in list_of_calories_per_elf] 

top_1 = sorted(((calories, elf) for elf, calories in enumerate(total_cal, 1)), reverse=True)[:1]

print(f"Top 1: {top_1}")

#TASK 1/PART 2: Find the top three Elves carrying the most Calories. How many Calories are those Elves carrying in total?

top_3 = sorted(((calories, elf) for elf, calories in enumerate(total_cal, 1)), reverse=True)[:3]

top_3_total_calories = [sum(i[0] for i in top_3)]

print(f"Top 3: {top_3}")
print(f"Total calories of top 3 elves: {top_3_total_calories}")
...
#win-6 points, lose-0 points, draw-3 points
...
#process the input so the input_data will be list of shapes played by each player
# For example:
# player1 = ['A', 'B', 'C']
# player2 = ['Y', 'X', 'Z']
...
total_score = shape_point + match_point
...
#TASK 2/PART 2: Following the Elf's instructions for the second column, what would your total score be if everything goes exactly according to your strategy guide?
...
total_score = task2_shape_point + task2_match_point
            
print(task2_shape_point)
print(task2_match_point)

print(f"Total score with the strategy used in second part of the task: {total_score}")


# In[140]:


input_data = """QtGHnGlcwQGvHwMHGnnGfRFCJnRBfVVJRVBfCFBh
...
#TASK 3/ PART 1: Find the item type that appears in both compartments of each rucksack. What is the sum of the priorities of those item types?

def split_list(rucksack):
    half = len(rucksack)//2
    return rucksack[:half], rucksack[half:]

def get_priority(item_type):
    # Reference for ord(): https://www.toppr.com/guides/python-guide/examples/python-examples/python-program-find-ascii-value-character/#:~:text=In%20Python%203%2C%20the%20ord,return%20the%20corresponding%20ASCII%20value.
    if 'a' <= item_type <= 'z':
        return ord(item_type) - ord('a') + 1
    elif 'A' <= item_type <= 'Z':
        return ord(item_type) - ord('A') + 27

def find_common_item_type(compartment1, compartment2):
    a = ''.join(set(compartment1)&set(compartment2))
...
for i in rucksacks:
    compartment1, compartment2 = split_list(i)
    total_priority+= find_common_item_type(compartment1, compartment2)
print(f"Total sum of priorities: {total_priority}")

#TASK 3/PART 2: Find the item type that corresponds to the badges of each three-Elf group. What is the sum of the priorities of those item types?

def find_badge_item_type(group):    
    a = ''.join(set(group[0])&set(group[1])&set(group[2]))
    return get_priority(a)
...
groups_of_three = [columns[i:i+3] for i in range(0, len(columns), 3)]

for group in groups_of_three:
    total_priority+= find_badge_item_type(group)
print(f"Total sum of badge priorities: {total_priority}")
